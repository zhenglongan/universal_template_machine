...
void init_motor( void )
{
    ...
    ifsr0 = 1;//1;//scx change r/f edeg  0;                          // rising/falling edge select,single edge
    ...
    switch( m_status )
    {
        ...
        case CLOSE_RUN:     //run of closed loop status
            ...
            else
            {
                ...
                if( ( sys.status == CHECKI04 ) && ( smotor_direction == 1 ) && ( motor.dir == 1 ) ) //主轴正反老化模式控制
                {
                    if( abs( m_spd_n ) > 2000 ) //1000rpm
                    {
                        temp16 = temp16 - 30 - ( ( abs( m_spd_n ) - 2000 ) >> 7 ); //3000rpm -60  按比例算 系数为0.015=1/64=0.0156  2倍转速，所以为1/128
                    }
                    else
                    {
                        temp16 = temp16 - 30; //-(INT16)STOP_KPs_2;
                    }
                }
...
#ifdef VOL_ADJUST
    //voltage adjust
    temp16 = ( ( INT32 )temp16 * DC300 ) / ( uzk_val ); //Q12
#endif
    temp16 += 54;//scx 72;
...
#ifdef VOL_ADJUST
    temp16 = ( ( INT32 )( temp16 * ( INT32 )DC300 ) ) / ( uzk_val ); //Q12
#endif
...
VECTOR rc_pc( INT16 h_y, INT16 h_x ) //scx 反正切， 输出值 为 角度值的4倍。模值经过电压修正
{
    ...
    switch( U_sign )
    {
        //case 0:;break;
        case 1:
            alpha_out =  360 - alpha_out;
            break;
        case 2:
            alpha_out =  720 - alpha_out;
            break;//x<0,y>=0
        case 3:
            alpha_out =  360 + alpha_out;
            break;
        case 4:
            alpha_out = 1440 - alpha_out;
            break;  //x>=0,y<0
        case 5:
            alpha_out = 1080 + alpha_out;
            break;
        case 6:
            alpha_out =  720 + alpha_out;
            break;  //x<0,y<0
...
void motor_control( void )
{
    ...
    INT16 d_iq = 0;//scx
    a_uzkval = ( ( INT32 )620 * 16384 ) / ( uzk_val ); //scx
    top = 255;
    bottom = 0;
    //testout ++;
    //da1 = testout;
